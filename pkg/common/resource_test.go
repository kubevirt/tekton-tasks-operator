...
var _ = Describe("Resource", func() {
	...
	BeforeEach(func() {
		s := scheme.Scheme
		Expect(tekton.AddToScheme(s)).ToNot(HaveOccurred())
	...
	Context("CreateOrUpdate", func() {
		...
		It("should set owner reference", func() {
			...
			Expect(owner.Kind).To(Equal("TektonTasks"))
		})
		...
		It("should set owner annotations", func() {
			...
			Expect(found.GetAnnotations()).To(HaveKeyWithValue(libhandler.TypeAnnotation, "TektonTasks.tektontasks.kubevirt.io"))
	...
	Context("Cleanup", func() {
		...
		It("should not delete, if the resource is not owned by tekton.R", func() {
			_, err := createOrUpdateTestResource(&request)
			Expect(err).ToNot(HaveOccurred())
