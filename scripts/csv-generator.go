...
func init() {
	...
	rootCmd.Flags().StringVar(&f.operatorNamespace, "operator-namespace", "", "Namespace in which tekton operator will be deployed (required)")
...
func updateContainerEnvVars(flags generatorFlags, container v1.Container) []v1.EnvVar {
	...
	for _, envVariable := range container.Env {
		...
		case environment.CreateDatavolumeImageKey:
			if flags.createDatavolumeImage != "" {
				envVariable.Value = flags.createDatavolumeImage
			}
